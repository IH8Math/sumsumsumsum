  ...
  app.post("/api/sheets", async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        res.status(500).json({ error: "Google Apps Script 설정 오류: Web App이 JSON이 아닌 HTML을 반환했습니다. 앱스스크립트 배포 시 '접근 권한: 모든 사용자(Anyone)'로 설정했는지 확인하세요.", details: text.substring(0, 100) });
      }
    } catch (error: any) {
