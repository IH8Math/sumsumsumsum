...
async function startServer() {
  ...
  app.post("/api/sheets", async (req, res) => {
    ...
    const gasUrl = (clientGasUrl && clientGasUrl.trim() !== "") ? clientGasUrl.trim() : (process.env.GAS_URL || "");

    if (!gasUrl || gasUrl.includes("YOUR_") || !gasUrl.startsWith("http")) {
      // Mock mode fallback when no valid URL provided
