  ...
  const handleDeleteCompletion = async (id: string, courseName: string, name: string) => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        const errorMsg = data.error || '구글 Apps Script 코드가 이전 버전이어서 이수 내역 삭제를 처리하지 못했습니다. 관리자 모드 [구글 시트 연동 설정] 탭에서 최신 코드를 복사하여 Apps Script에 [새 배포]를 진행해 주세요.';
        toast.error(`시트 삭제 실패: ${errorMsg}`, { duration: 8000 });
      }
      onRefresh();
    } catch (error: any) {
